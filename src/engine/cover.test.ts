...
describe('buildCover', () => {
  it('keeps one-layer summaries and a structure verdict', () => {
    ...
    expect(cover.summaries.why).toMatch(/行为问题|分组差异/)
    expect(cover.summaries.why).not.toMatch(/个主因/)
    expect(cover.luckLine).not.toMatch(/体系偏亏/)
    expect(cover.summaries.how).toMatch(/改善|依据|亏损|可靠|退出/)
    expect(cover.summaries.how).not.toMatch(/可采用|继续观察|理论上限|当前不支持/)
    expect(cover.howLead.problem).toMatch(/损失主要集中在/)
    expect(cover.howLead.action).toMatch(/单笔亏损|持仓天数/)
    expect(cover.howLead.limit).toMatch(/当前账本|追踪承诺/)
    ...
    if (hold) {
      ...
      expect(hold.capLines?.join(' ')).toMatch(/机械上限/)
      expect(hold.finding).toMatch(/可实现/)
    }
